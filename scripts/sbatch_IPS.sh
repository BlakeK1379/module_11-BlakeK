...
#SBATCH --mail-user=karavas.b@northeastern.edu # sbatch job progress will be sent via email to this address

# Usage: sbatch scripts/sbatch_IPS.sh 
...
INPUT_DATA_PATH=/work/courses/BINF6308/AiptasiaRNASeq/data/predictedProteins
...
echo "Building a directory for shell scripts."
mkdir -p ./scripts
echo "Building a directory to store raw protein data."
mkdir -p ./data 
echo "Building a directory for analysis results."
mkdir -p ./results
echo "Building a directory for storing .log and .err files."
mkdir -p ./results/logs/
echo "Building a directory for storing pipeline analyses."
mkdir -p ./analysis
echo
echo

echo "Tracking new directories in git by adding brief README.md files."
...
echo "Stores analyses of pipeline runs. These will be tracked."> analysis/README.md
echo
echo

echo "Moving predicted proteins to the working directory."
cp -u $ALL_PROTEINS data/
